...
(function() {
    ...
    // Generate the form depending on parameters (it is ugly but eh)
        ...
        if (LoginForm.Parameters.allowAccountCreation) {
            html = html +
                        "<br>" +
                        "<input type=\"password\" id=\"inputPassword\" placeholder=\"Password\" class=\"login-input\">";
        }
        ...
        $('head').append('<link rel="stylesheet" type="text/css" href="../../css/LoginForm.css">');
    ...
    LoginForm.prototype.disableForm = function() {
        this.displayError("A conexão com o servidor foi perdida.");
    };
    ...
    LoginForm.prototype.connectAttempt = function() {
        ...
        if (payload.username.length <= 0) {
            return this.displayError("Você deve utilizar um nome de usuário!");
        }
        if (payload.username.includes(" ")) {
            return this.displayError("Espaços são proibidos no nome de usuário!");
        }
        if (!payload.username.match(/^(?=[a-zA-Z0-9\s]{2,25}$)(?=[a-zA-Z\s])(?:([\w\s*?])\1?(?!\1))+$/)) {
            return this.displayError("Você não pode utilizar caracteres especiais no nome de usuário.");
        }
